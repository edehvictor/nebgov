...
mod test {
    ...
    impl MockTarget {
        pub fn exec(env: Env) {
            env.storage().instance().set(&symbol_short!("executed"), &true);
        }
        
        pub fn was_executed(env: Env) -> bool {
            env.storage().instance().get(&symbol_short!("executed")).unwrap_or(false)
        }
    }
    ...
    fn test_compute_op_id() {
        ...
        let op_id1 = TimelockContract::compute_op_id(&env, &target, &data, &Bytes::new(&env), &Bytes::new(&env));
        let op_id2 = TimelockContract::compute_op_id(&env, &target, &data, &predecessor, &salt);
        let op_id3 = TimelockContract::compute_op_id(&env, &target, &data, &predecessor, &salt);
        let op_id4 = TimelockContract::compute_op_id(&env, &target, &data, &Bytes::new(&env), &salt);
    ...
    fn test_schedule_with_predecessor_and_salt() {
        ...
        client.initialize(&admin, &governor, &86400);
        ...
        let op_id = client.schedule(&governor, &target, &data, &fn_name, &delay, &predecessor, &salt);

        // Verify operation is stored
        let op: Operation = env.storage().persistent().get(&DataKey::Operation(op_id.clone())).unwrap();
    ...
    fn test_schedule_validates_predecessor() {
        ...
        let result = std::panic::catch_unwind(|| {
            client.schedule(&governor, &target, &data, &fn_name, &delay, &non_existent_pred, &Bytes::new(&env));
        });
    ...
    fn test_is_done() {
        ...
        let op_id = client.schedule(&governor, &target, &data, &fn_name, &delay, &Bytes::new(&env), &Bytes::new(&env));

        // Not done yet
        assert!(!client.is_done(&op_id));

        // Execute it
        client.execute(&governor, &op_id).unwrap();
    ...
    fn test_predecessor_enforcement() {
        ...
        let op_a_id = client.schedule(&governor, &target, &data, &fn_name, &0u64, &Bytes::new(&env), &Bytes::new(&env));

        // Schedule operation B with A as predecessor
        let op_b_id = client.schedule(&governor, &target, &data, &fn_name, &0u64, &op_a_id.clone(), &Bytes::new(&env));
        ...
        client.execute(&governor, &op_a_id).unwrap();
        assert!(client.is_done(&op_a_id));

        // Now B should execute successfully
        client.execute(&governor, &op_b_id).unwrap();
    ...
    fn test_salt_uniqueness() {
        ...
        let op_id1 = client.schedule(&governor, &target, &data, &fn_name, &delay, &Bytes::new(&env), &salt1);
        let op_id2 = client.schedule(&governor, &target, &data, &fn_name, &delay, &Bytes::new(&env), &salt2);

        assert_ne!(op_id1, op_id2);

        // Both operations should coexist in storage
        let op1: Operation = env.storage().persistent().get(&DataKey::Operation(op_id1.clone())).unwrap();
        let op2: Operation = env.storage().persistent().get(&DataKey::Operation(op_id2.clone())).unwrap();
    ...
    fn test_same_salt_same_id() {
        ...
        let op_id1 = client.schedule(&governor, &target, &data, &fn_name, &delay, &Bytes::new(&env), &salt);
        let op_id2 = client.schedule(&governor, &target, &data, &fn_name, &delay, &Bytes::new(&env), &salt);

        assert_eq!(op_id1, op_id2);

        // The second schedule should overwrite the first (same storage key)
        let op: Operation = env.storage().persistent().get(&DataKey::Operation(op_id1.clone())).unwrap();
    ...
    fn test_predecessor_changes_id() {
        ...
        let op_id1 = client.schedule(&governor, &target, &data, &fn_name, &delay, &pred1, &salt);
        let op_id2 = client.schedule(&governor, &target, &data, &fn_name, &delay, &pred2, &salt);
    ...
    fn test_empty_predecessor_and_salt() {
        ...
        let op_id1 = client.schedule(&governor, &target, &data, &fn_name, &delay, &Bytes::new(&env), &Bytes::new(&env));
        let op_id2 = client.schedule(&governor, &target, &data, &fn_name, &delay, &Bytes::new(&env), &Bytes::new(&env));
